...
def test_versions(mock_path_exists):
    ...
    ) == ("owner", "repo", "path/to/skill", "v4", True)
...
def test_malformed_urls_injection(mock_path_exists):
    ...
    # urllib.parse handles auth in netloc, which breaks parsed.netloc == "github.com"
    assert parse_repo_arg("https://user:pass@github.com/owner/repo") is None
